...
void print_exp ( jEXP* p ) {
  ...
  if( p ) {
    ...
    case eLT: print_exp( p->exp1 );
	      ...
	      print_exp( p->exp2 );
	      break;
    case eLE: print_exp( p->exp1 );
	      fprintf(yyout, " <= ");
	      print_exp( p->exp2 );
	      break;
    case eEQ: print_exp( p->exp1 );
	      fprintf(yyout, " == ");
	      print_exp( p->exp2 );
	      break;
  ...
  if( p ) {
    ...
    case sPROG: fprintf(yyout, "#include <stdio.h>\n\n");
		print_stm( p->stm2 );
	        print_stm( p->stm1 );
	        break;
    case sMAINC: fprintf(yyout, "int main() {\n");
		 print_stm( p->stm1 );
		 fprintf(yyout, "}\n");
	         break;
    ...
    case sSTMTS: print_stm( p->stm1 );
		 ...
		 fprintf(yyout, "\n");
	         break;
    case sBSTMT: fprintf(yyout, "{");
		 print_stm( p->stm1 );
		 print_stm( p->stm2 );
		 fprintf(yyout, "}\n");
	         break;
    case sIF: fprintf(yyout, "if ( ");
	      print_exp( p->exp1 );
	      fprintf(yyout, " )\n");
	      print_stm( p->stm1 );
	      fprintf(yyout, "else\n");
	      ...
	      fprintf(yyout, "\n");
	      break;
    case sWHILE: fprintf(yyout, "while ( ");
		 print_exp( p->exp1 );
		 fprintf(yyout, " )\n");
		 print_stm( p->stm1 );
		 fprintf(yyout, "\n");
		 break;
    case sPRINT: fprintf(yyout, "printf(\"%%d\\n\", ");
		 print_exp( p->exp1 );
		 fprintf(yyout, ");\n");
		 break;
    case sASSIGN: print_exp( p->exp1 );
		  fprintf(yyout, " = ");
		  print_exp( p->exp2 );
		  fprintf(yyout, ";\n");
		  break;
    case sARRASS: te = p->exp1;
		  fprintf(yyout, "%s\[", te->name);
		  print_exp( te->exp1 );
		  fprintf(yyout, "] = ");
		  print_exp( p->exp2 );
